...
if ((!dirw) && (!DirMemory))
  ...
  //if (Actualgear == 5)
    ...
    EEPROM.write(1, Motorstatus); // Write Motorstatus to EEPROM  
    }
  if (Motorstatus == 2)
    {
...
if ((!gearw) && (!GearMemory))
  ...
  if (Motorgear == 1)
    ...
    digitalWrite(A4988MS3,HIGH); // SixteenthStep
    ReturnGear = 1;
    EEPROM.write(4, ReturnGear); // Write ReturnGear to EEPROM
    }
  if (Motorgear == 2)
    {
    digitalWrite(A4988MS1,HIGH); // EightStep 
...
void setup() {
 ...
 digitalWrite(MOTORDIRPIN, HIGH);
 Motorstatus = EEPROM.read(1); 
 Motorgear = EEPROM.read(2); 
 if (Motorstatus > 3) { Motorstatus = 0; }
 if (Motorgear > 3) { Motorgear = 1; }
